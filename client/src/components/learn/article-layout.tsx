      ...
      <div className="min-h-screen bg-black">
        ...
        <article className="container max-w-2xl mx-auto px-4 pt-24 pb-6">
          ...
          <nav
            ...
            className="flex items-center flex-wrap gap-1 text-xs text-white/45 mb-5"
          >
          ...
          <div className="bg-white/10 border border-white/20 rounded-xl p-5 mb-6">
            ...
            <div className="text-white/85 text-sm leading-relaxed">{shortAnswer}</div>
          </div>
          ...
          {toc.length > 0 && (
            <nav className="bg-white/5 border border-white/10 rounded-xl p-5 mb-8">
              <p className="text-white text-sm font-semibold flex items-center gap-2 mb-3">
                <ListTree className="h-4 w-4 text-white" /> On this page
              </p>
              <ol className="space-y-2">
                {toc.map((item) => (
                  <li key={item.id}>
                    <a
                      href={`#${item.id}`}
                      className="text-white/70 hover:text-white text-sm transition-colors inline-flex items-start gap-1.5"
                    >
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 text-white flex-shrink-0" />
                      {item.label}
                    </a>
                  </li>
                ))}
                ...
                {relatedArticles.map((a) => (
                  ...
                  >
                    <div className="bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/30 rounded-xl p-4 transition-all group cursor-pointer">
                      ...
                      <p className="text-white/55 text-xs leading-relaxed">{a.excerpt}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {/* CTA → teamdill.com funnel (UTM-tagged) */}
          <div className="bg-white/10 border border-white/20 rounded-xl p-6 text-center">
            ...
            <p className="text-white/55 text-sm mb-4">{cta.body}</p>
            <a
              href={ctaHref}
              target="_blank"
            ...
            >
              <Button className="w-full bg-white hover:bg-[#0099bb] text-white font-semibold flex items-center justify-center gap-2">
                {cta.label}
