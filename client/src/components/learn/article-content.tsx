...
export function P({ children }: { children: ReactNode }) {
  return <p className="text-white/70 text-sm leading-relaxed">{children}</p>;
}
...
export function SourceLink({ href, children, source }: SourceLinkProps) {
  ...
  return (
    <a
      ...
      className="text-white hover:text-white underline inline-flex items-center gap-0.5 transition-colors"
    >
      {children}
...
export function StatCallout({ children }: { children: ReactNode }) {
  return (
    <blockquote className="border-l-2 border-white bg-white/5 rounded-r-lg pl-4 pr-3 py-3 my-4">
      <p className="text-white/80 text-sm leading-relaxed italic">{children}</p>
    </blockquote>
  );
}
      ...
      <table className="w-full text-left border-collapse text-xs sm:text-sm">
        <thead>
          <tr className="border-b border-white/20">
            <th className="py-3 pr-3 text-white/55 font-semibold align-bottom">
              {rowHeader}
            </th>
            {columns.map((col) => (
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.label} className="border-b border-white/10 align-top">
              <td className="py-3 pr-3 text-white/80 font-medium">{row.label}</td>
              {columns.map((col) => (
                <td key={col.key} className="py-3 px-3 text-white/70">
                  {row.values[col.key]}
                </td>
              ))}
            </tr>
