...
const TITLE = "Learn: Mortgage Education Hub | CamDill";
const DESCRIPTION =
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="container max-w-2xl mx-auto px-4 pt-24 pb-6">
            ...
            return (
              <section key={cat.id} id={cat.id} className="scroll-mt-20">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {cat.focus && (
                    <span className="inline-flex items-center gap-1 bg-white/15 border border-white/20 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full">
                      <Star className="h-2.5 w-2.5" /> Focus
                    </span>
                  )}
                </div>
                <p className="text-white/45 text-sm mb-4">{cat.description}</p>
                    ...
                    {CALCULATOR_LINKS.map((c) => (
                      ...
                      >
                        <div className="bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/30 rounded-xl p-4 transition-all group cursor-pointer flex items-center gap-3">
                          <Calculator className="h-5 w-5 text-white flex-shrink-0" />
                    ...
                    {articles.map((a) => (
                      <Link key={a!.slug} href={`/learn/${a!.slug}`}>
                        <div className="bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/30 rounded-xl p-5 transition-all group cursor-pointer">
                          <h3 className="text-white font-bold text-base leading-snug mb-1.5 group-hover:text-white transition-colors">
                            {a!.title}
                          </h3>
        ...
        {/* CTA */}
        <div className="mt-12 bg-white/10 border border-white/20 rounded-xl p-6 text-center">
          ...
          <Button
            onClick={openCal}
            className="bg-white hover:bg-[#0099bb] text-white font-semibold px-6 flex items-center justify-center gap-2 mx-auto"
          >
